...
!!! version 3, altitude affects correlation
...
subroutine kriegepred3(x,y,b,grid, ypred, lsm, lsmy, alt, alty, cy,cgrid,nobs,npar,ngrid, covpars)
  implicit none
  
  !  include 'lapack_inc.f90'
  integer, parameter :: Double = selected_real_kind(15)
  integer, parameter :: dbl=Double
  integer, parameter :: ncovpars = 3

  ! altitude correlation length (m) must match r code now!!
!  real(kind=dbl), parameter :: altlen = 200d0

  integer, intent(in) :: nobs, npar, ngrid
  real(kind=dbl), intent(inout) :: ypred(ngrid) 
  real(kind=dbl), intent(inout) :: x(nobs,npar), y(nobs,1), b(nobs,nobs)
  real(kind=dbl), intent(in) :: grid(ngrid,npar), lsm(ngrid), lsmy(nobs), alt(ngrid), alty(nobs)
...
!! land sea mask
  ...
  !! land sea mask and altitude
  subroutine corrdist4(x1,x,b1, lsm, lsmx, alt, altx, n, covpars)
    implicit none
    real(kind=dbl), intent(in) :: x1(:), x(:,:), lsm, lsmx(:), alt, altx(:)
    integer, intent(in) :: n
    real(kind=dbl), intent(out) :: b1(n)
    real(kind=dbl), intent(in) :: covpars(ncovpars)
    ...
    integer :: i
    real(kind=dbl) :: d,a
    real(kind=dbl) :: sig2, clen, altlen
    ...
    do i=1,n
       d = sqrt((x1(1)-x(i,1))**2 + (x1(2)-x(i,2))**2)
!       a = exp(-(alt-altx(i))**2/altlen**2)
       a = exp(-abs(alt-altx(i))/altlen)
       if (lsm .eq. lsmx(i)) then
          b1(i) = a*exp(-d/clen)*sig2
       else
          b1(i) = 0.0d0
       end if          
    end do  
  end subroutine corrdist4
